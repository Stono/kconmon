const {
  convertKubeconfig
} = require('kubernetes-client/backends/request/config')
...
function refreshAuth(type, config) {
  return new Promise((resolve, reject) => {
    const provider = require(`kubernetes-client/backends/request/auth-providers/${type}.js`)
    provider
      .refresh(config)
      .then((result) => {
        const auth = {
      ...
      if ((res.statusCode === 401 || res.statusCode === 403) && auth.type) {
        return refreshAuth(auth.type, auth.config)
          .then((newAuth) => {
            ...
            return request(options, (requestErr, requestRes, requestBody) => {
              if (requestErr) {
          ...
          .catch((refreshAuthErr) => cb(refreshAuthErr))
      }
